...
export class GameDetailComponent {
    render(data: GameData, gameName?: string | null) {
        ...
        const finishDateHtml = game.finish_date ? `<p><strong>Finished Date:</strong> <span id="detail-finish-date">${escapeHtml(game.finish_date)}</span></p>` : "";
        return `<div id="game-detail-view"><a href="#all-games" class="back-link">Back to List</a>${iconHtml}<h2 id="detail-game-name">${escapeHtml(game.name)}</h2><div class="game-info"><p><strong>Total Playtime:</strong> <span id="detail-playtime">${formatPlaytime(game.play_time || 0)}</span></p><p><strong>Sessions:</strong> <span id="detail-sessions">${game.session_count}</span></p><p><strong>Status:</strong> <span id="detail-status">${escapeHtml(game.status)}</span></p>${finishDateHtml}<p><strong>Last Played:</strong> <span id="detail-last-played">${escapeHtml(game.last_play_date || "N/A")}</span></p></div><h3>Session History</h3><table><thead><tr><th>Start Time</th><th>Duration</th></tr></thead><tbody>${sessionRows}</tbody></table></div>`;
    }
}
